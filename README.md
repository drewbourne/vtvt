...
- `@fbt/ui`: a stub React component library shared by both `web` and `docs` applications
- `@fbt/eslint-config`: `eslint` configurations (includes `eslint-config-next` and `eslint-config-prettier`)
- `@fbt/typescript-config`: `tsconfig.json`s used throughout the monorepo
